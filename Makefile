...
KIND_CLUSTER ?= portail-operator-test-e2e
...
.PHONY: image-buildx
image-buildx: ## Build and push multi-platform container image.
	$(CONTAINER_TOOL) manifest create ${IMG}
	for platform in $$(echo $(PLATFORMS) | tr ',' ' '); do \
		$(CONTAINER_TOOL) build --platform $$platform -t ${IMG}-$$platform -f Containerfile . && \
		$(CONTAINER_TOOL) manifest add ${IMG} containers-storage:${IMG}-$$platform; \
	done
...
bundle: manifests kustomize ## Generate OLM bundle manifests and metadata.
	...
	"$(KUSTOMIZE)" build config/manifests | operator-sdk generate bundle -q \
		...
		$(if $(BUNDLE_DEFAULT_CHANNEL),--default-channel $(BUNDLE_DEFAULT_CHANNEL))
	@if [ -f "$(BUNDLE_ICON)" ]; then \
		echo "Injecting icon from $(BUNDLE_ICON)..."; \
